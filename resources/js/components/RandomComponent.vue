...
<div class="w-25">
    ...
    <div class="w-100 svg_container">
        <svg xmlns="" xmlns:xlink="" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
            ...
            <!--Text & link to path-->
            <text font-family="monospace" font-size="20" fill="#D91E18">
                <textPath id="text" xlink:href="#s">{{ response_data }}</textPath>
            </text>
        </svg>
    </div>
</div>
    ...
    methods: {
        getData: function() {
            axios.get('api/random').then((res) => {
                this.response_data = res.data.map((item)=> item.values).join(" ");
            }).catch((error) => {
                console.log(error);
            })
        },
