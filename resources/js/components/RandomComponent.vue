...
<div class="w-100">
    ...
    <div id="flex_container">
      <div class="first_container">
        ...
        <svg xmlns="" xmlns:xlink="" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
            <!--Background-->
            ...
            <!--Text & link to path-->
            <text font-family="monospace" font-size="15" fill="#D91E18">
                <textPath id="text_random" xlink:href="#s">{{ response_random }}</textPath>
            </text>
        </svg>
      </div>
      <div class="second_container">
        <label>Breakdown Value:</label>
        <svg xmlns="" xmlns:xlink="" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
            <!--Background-->
            <path d="M0 0h728v400H0z" fill="#fff" />
            <!--Spiral path-->
            ...
            <!--Text & link to path-->
            <text font-family="monospace" font-size="15" fill="#D91E18">
                <textPath id="text_breakdown" xlink:href="#s">{{ response_breakdown }}</textPath>
            </text>
        </svg>
      </div>
    </div>
    ...
    methods: {
        getData: function() {
            axios.get('api/random').then((res) => {
                this.response_random = res.data.map((item)=> item.values).join(" ");
                this.response_breakdown = res.data.map((item)=> item.random[0].values).join(" ");
            }).catch((error) => {
                console.log(error);
            })
        },
